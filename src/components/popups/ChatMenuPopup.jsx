...
const ChatMenuPopup = ({ options, className = "" }) => {
  return (
    <motion.div initial={{ scaleY: 0, opacity: 0 }} animate={{ scaleY: 1, opacity: 1 }} exit={{ scaleY: 0, opacity: 0 }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1],}} style={{ originY: 0 }}  className={` bg-white shadow-xl shadow-black/10 min-w-44 p-2 grid gap-1 rounded-xl z-[100] border border-zinc-100 ${className}`}>
      {options.map((option) => (
