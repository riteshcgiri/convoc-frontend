...
const TextInput = ({ label, error, register, name, type = "text", parentClass, labelClass, inputClass, frontIcon, rearIcon, placeholder='' }) => {
    ...
    return (
        <div className={`relative w-full ${parentClass}`}>
            ...
            <div className={`w-full flex  items-center px-4  border-2 rounded-xl ${error ? "border-red-500 " : "border-primary focus-within:bg-primary/15 "} ${inputClass}`}>
                {frontIcon && frontIcon}
                <input type={type} {...register(name)} id={name} name={name} className={`w-full py-4 outline-none transition text-primary font-semibold`} placeholder={placeholder} autoComplete="off" autoCapitalize="off" />
